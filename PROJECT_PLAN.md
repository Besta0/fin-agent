...
- 输出下一步可执行动作，降低用户不知道该问什么、点哪里、先做什么的成本
...
- Chainlit 工作台提供快捷按钮：报告库、观察池、模型设置和当前优先标的分析
...
- 让用户打开应用后先看到产品介绍页，而不是直接进入聊天或长帮助文本
- 用产品语言解释项目价值、核心功能、研究流程和适用场景
- 把“这个项目是什么”讲清楚，再引导用户进入工作台、新建研究或配置模型
- 提供更像投研 SaaS 的默认视觉体验

当前实现：

- `app.py` 中 `_format_product_landing()` 渲染产品主页，介绍产品价值、核心功能、研究流程和当前环境
- `app.py` 中 `_format_product_home()` 渲染工作台首页，展示状态栏、任务中心、工作区和多 Agent 研究流水线
- 产品主页快捷动作包括进入工作台、分析当前优先标的、模型设置和能力指南
- 工作台任务中心根据当前 provider、API key、观察池和报告库状态，提示用户先配置模型、测试连接、启动分析或复盘报告
- 工作台快捷动作根据状态动态变化：未配置 API key 时优先显示配置和测试；配置完成后优先显示当前观察池 Top 标的分析、报告库、观察池和模型设置
- 研究报告完成后自动提供“打开报告 / 重新分析 / 报告库 / 观察池 / 模型设置”等后续动作
- 报告库、观察池、模型设置和连接测试都能回到产品主页或工作台，形成闭环导航
