...
- 展示当前模型配置、研究资产数量、工作流阶段和上下文动作
- 把“能做什么”转化成“下一步做什么”，让用户不用记命令也能开始
- 提供更像投研 SaaS 的默认视觉体验

当前实现：

- `app.py` 中 `_format_product_home()` 渲染 Home
- 首页展示产品定位、状态栏、任务中心、工作区和多 Agent 研究流水线
- 任务中心根据当前 provider、API key、观察池和报告库状态，提示用户先配置模型、测试连接、启动分析或复盘报告
- 首页快捷动作根据状态动态变化：未配置 API key 时优先显示配置和测试；配置完成后优先显示当前观察池 Top 标的分析、报告库、观察池和模型设置
- 研究报告完成后自动提供“打开报告 / 重新分析 / 报告库 / 观察池 / 模型设置”等后续动作
- 报告库、观察池、模型设置和连接测试都能回到首页，形成闭环导航
