...
Chainlit 首页被重新设计为一个投研工作台，而不是默认聊天帮助页：

- 顶部定位：明确说明这是“新建研究 → 观察池跟踪 → 报告复盘”的中文多 Agent 股票研究工作区
- 状态栏：展示运行状态、provider、model、base_url、API key 状态、观察池、报告库和记忆数量
- 任务中心：根据当前配置自动给出下一步动作，例如先配置模型、测试连接、打开观察池或复盘最新报告
- 工作区：把新建研究、报告库、观察池、模型设置固定成四个核心入口
- 研究流水线：展示标的识别、数据采集、分析辩论、报告质检、观察池沉淀的完整路径
- 上下文按钮：未配置 API key 时优先显示“配置模型 / 测试连接”，配置完成后优先显示“分析 Top 标的 / 报告库 / 观察池”
