...
Chainlit 现在有两层产品入口：产品主页负责介绍价值和功能，投研工作台负责进入日常研究任务。

- 产品主页：介绍项目定位、用户痛点、核心功能、研究流程和当前环境状态
- 进入工作台：展示运行状态、provider、model、base_url、API key 状态、观察池、报告库和记忆数量
...
- 独立看板运行前预检：启动前检查 ticker、模型、API key、预计 Agent 和风险提示，前端按钮和后端 `/api/run/start` 都会阻断不合格请求；预检通过时可展开查看将启动的 Agent 队列
...
Intent Router
  ...
  ├─ Run Preflight → Ticker / Model Readiness / Estimated Agents / Warnings
...
文件：`financial_agent/tools/run_preflight.py`
...
- 给前端提供 `decision` 和 `estimated_agents`，用于展示阻断原因和启动前 Agent 队列预览
...
- 独立看板支持运行前预检卡，展示识别标的、模型、API key 状态、预计 Agent 数、阻断原因、warnings，以及可展开的 Agent 启动预览
...
- 自动渲染“报告库”：按时间展示最近报告，并显示评级、置信度、分析周期、价格、质检状态和资料链接数量
- 自动渲染“报告阅读页”：结论面板、市场与技术、基本面、多空与投委会、风险与观察指标、新闻线索、历史记忆与观察池、质量检查、正文目录和下一步动作
- 支持把当前报告导出为独立 HTML，便于分享、归档或离线阅读
...
- Agent 队列可以按全部、运行中、已完成、待处理、失败筛选
- Agent 详情面板可以按字段或内容过滤结构化输出，例如搜索 `PE`、`risk`、`news`
- “模型设置”面板选择 provider、model、base_url、API key 和 temperature
- “测试连接”验证当前模型配置
- 观察每个 Agent 的输出、协作时间线、多空辩论、历史对比和报告正文，并可把报告导出成 HTML
...
顶部 **观察池** 会显示 Portfolio Agent 自动维护的研究队列；点击标的卡片里的 **复盘**，会把复盘问题填到输入框。

如果同一用户对同一 ticker 有多次分析，顶部 **历史对比** 会显示当前 run 与上一条 run 的结论、置信度、价格和近 1 月表现变化。
...
- 独立看板实时运行态：展示当前 Agent、节点耗时、总耗时、失败节点、错误详情和 Agent 状态筛选
- 独立看板详情过滤：按字段名或内容检索单个 Agent 的结构化输出
